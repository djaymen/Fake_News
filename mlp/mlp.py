...
class FNC(object):
    ...
    def __init__(self,arch='3L_32x2',n_nodes=config['n_nodes'],factor=config['factor'],
                n_layers=config['n_layers'],l2_reg=config['l2_reg'],dropout=config['dropout']
                dim=20672) -> None:
        super().__init__()
        self.arch = arch
    ...
    def build_model(self,n_blocks=config.n_res_blocks):
        # Input Block
        inp = Input(shape=(self.dim, 1), name='Input')
    ...
    def body_block(self,x):
        ...
        for i in range(self.n_layers):
            x = layers.Dense(n*f,kernel_regularizer=self.l2_reg)(x)
            x = self.activation(x)
        return x
        
    def output_block(self,x):
        return layers.Activation('softmax', name='Output')(layers.Flatten()(x))
